...
Stránka **Můj profil** (v menu ve skupině **Ostatní**, `/admin/other/profile`) je samostatná od
Uživatelských nastavení popsaných níže — najdete tu jméno, e-mail a dvě věci, které se pak zobrazují
u vašeho jména po celé aplikaci (v přihláškách, financích, na tržišti, v e-mailech…):

![Můj profil — avatar a barva odznaku](./img/muj-profil-avatar.png)

- **Barva badge** — barva kulatého odznaku s vašimi iniciálami, kterým jste v systému označeni,
  pokud nemáte nahraný profilový obrázek
- **Profilový obrázek** — čtvercový obrázek nahraný přetažením (drag&drop) nebo tlačítkem
  **Vyhledat**; ořez proběhne rovnou při nahrávání. Bez obrázku se zobrazí barevný odznak s iniciálami
  v barvě, kterou jste zvolili výše
